...
const TypingTest = ({ resetToken, selectedLanguage }: TypingTestProp) => {
  ...
  const totalLetters = lines.reduce(
    (sum, line) => sum + line.words.reduce((s, w) => s + w.length, 0),
    0,
  );
  ...
  // maybe just move this into the line thing later
  const isLineComplete = (lineIndex: number) => {
    ...
    for (let wordIndex = 0; wordIndex < wordCount; wordIndex++) {
      const wordRef = lineRef.getWord(wordIndex);

      if (!wordRef) return false;

      const letterCount = wordRef.getLetterCount();

      for (let letterIndex = 0; letterIndex < letterCount; letterIndex++) {
        const letterRef = wordRef.getLetter(letterIndex);

        if (!letterRef) return false;

        if (letterRef.getStatus() == "pending") return false;
      }
    }
  ...
  const handleKeyDown = useCallback(
    (event: KeyboardEvent) => {
      ...
      const lettersTypedBeforeCurrentLine = lines
        ...
        .reduce(
          (sum, lineObj) =>
            sum + lineObj.words.reduce((wSum, w) => wSum + w.length, 0),
          0,
        );
      ...
      if (key === "Backspace") {
        setLetterIndex((prevLetterIndex) => {
          ...
          setLetterStatus(lineIndex, wordIndex, newLetterIndex, "", "pending");
          return newLetterIndex;
        });
        return;
      }
      ...
      if (key === "Enter") {
        // add a check
        //no next line unless all leters typed

        if (!isLineComplete(lineIndex)) return;
